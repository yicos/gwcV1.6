	...
	public void run() {
		...
		try {
			...
			if (null != carRuntimeInfoTasks && carRuntimeInfoTasks.size() > 0) {
				/*LogListener.debug(this.getClass(), AppConstant.NAME_CMD_UPLOADLOCATIONS + msgThreadType,
						"要发送记录集合=" + carRuntimeInfoTasks);*/
		...
		for (int i = 0; i < carRuntimeInfoTasks.size(); i++) {
			try {
				...
				if (i == 0 || i % 20 != 0) {
					...
					CarRuntimeInfoTask taskReq = new CarRuntimeInfoTask();
				...
				// 组装消息格式
				CommonReq commonRsp = new CommonReq(AppConstant.CMD_UPLOADLOCATIONS, carRuntimeInfoTaskReqs);
