...
public class GetTaxiModelAndTaxiTypeRunable implements Runnable {
	...
	private Map<String, TaxiModel> getTaxiModel() throws Exception {
		...
		CommonRsp GetTaxiModelCommonRsp = new CommonRsp(AppConstant.GetTaxiModel, new GetTaxiAndTypeReq(AppConstant.PLATFORMID));
		...
		JSONObject GetTaxiModelJson =null;
		try {
			GetTaxiModelJson = JSONObject.fromObject(GetTaxiModel);
		} catch (Exception e) {
			throw new Exception("返回格式错误="+GetTaxiModel);
		}
	...
	private Map<String, TaxiType> getTaxiType() throws Exception {
		...
		CommonRsp getTaxiTypeCommonRsp = new CommonRsp(AppConstant.GetTaxiType, new GetTaxiAndTypeReq(AppConstant.PLATFORMID));
		...
		JSONObject getTaxiTypeJson =null;
		try {
			getTaxiTypeJson = JSONObject.fromObject(getTaxiType);
		} catch (Exception e) {
			throw new Exception("返回格式错误="+getTaxiType);
		}
